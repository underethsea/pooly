...
async function player(address) {
    ...
    try {
        ...
        let [
            ...
            CONTRACTS.TICKET.OPTIMISM.balanceOf(address),
            CONTRACTS.TICKET.POLYGON.getAverageBalanceBetween(
                address,
                avaxTicketStartTimestamp,
                currentTimestamp
        ...
        let balanceRemainingOptimism = parseFloat(
            ethers.utils.formatUnits(balanceOptimism, 6)
        );
        let optimismDelegatedBalance = await DelegatedBalance(address, 6);
        playerDataOptimism = playerData.filter(function (entry) {
            return entry.network === "optimism";
        });

        let experienceOptimism = playerDataOptimism.length;

        if (experienceOptimism > 0) {
            gotOne = 1;
            ...
            withdrawString += "   XP  " + experienceOptimism + " draws ";

            let totalClaimable = 0;
            for (let x of playerDataOptimism) {
                let total = 0;
                let prize = 0;
        ...
        if (balanceRemainingOptimism > 1) {
            withdrawString +=
                "\n Tickets Held" + emoji("usdc") + " " + Commas(balanceRemainingOptimism);
        }
        if (optimismDelegatedBalance > balanceRemaining) {
            withdrawString +=
                "\n Tickets + Delegation " +
                emoji("usdc") +
                " " +
                Commas(parseFloat(optimismDelegatedBalance));
        }
